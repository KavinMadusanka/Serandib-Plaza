...
const LowLevelStock = () => {
    ...
    const getAllProducts = async () => {
        try {
            ...
            const lowStockProducts = data.products.filter(p => p.quantity > 0 && p.quantity <= p.reorderLevel); // Filter for low stock only
            setProducts(data.products);
            setFilteredProducts(lowStockProducts); // Only set low stock products
        } catch (error) {
            console.log(error);
    ...
    const handleCategoryChange = (value) => {
        ...
        if (value) {
            const filtered = products.filter((p) => p.category.name === value && p.quantity > 0 && p.quantity <= p.reorderLevel); // Filter for low stock only
            setFilteredProducts(filtered);
        } else {
            setFilteredProducts(products.filter(p => p.quantity > 0 && p.quantity <= p.reorderLevel)); // Show all low stock products if no category is selected
        }
    };
    ...
    const filterProducts = (category, query) => {
        let filtered = products.filter(p => p.quantity > 0 && p.quantity <= p.reorderLevel); // Start with low stock products
    ...
    const generateReportWithoutImages = () => {
        ...
        doc.save('Inventory_Report(Low_Level_Stock).pdf');
    };     
    ...
    const generateExcelReport = () => {
        ...
        XLSX.writeFile(workbook, 'Inventory_Report(Low_Level_Stock).xlsx');
    };
